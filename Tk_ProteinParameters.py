...
class Protein:
    ...
    def calculate_ext_coef(self):  
        ...
        ext_coef_cformed = (
            self.aa_dict.get("Y", 0) * 1490
            + self.aa_dict.get("W", 0) * 5500
            + self.aa_dict.get("C", 0) // 2 * 125
        )
        ext_coef_reduced = (
            self.aa_dict.get("Y", 0) * 1490 + self.aa_dict.get("W", 0) * 5500
        )
        abs_cformed = ext_coef_cformed / self.mol_weight
        abs_reduced = ext_coef_reduced / self.mol_weight
    
        self.ext_coef = {
            "ext_coef_cformed": ext_coef_cformed, 
...
def save_file():
    """Saves the data from result window to a .txt file"""
...
def on_closing_result_window():  
    """Protects the result window from closing (result_window.protocol)"""
    
    pass

def copy_paste(e):  
    """Fixes the hotkeys copy-paste problem when you use RU keyboard"""
